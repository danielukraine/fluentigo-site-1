...
function TeacherCard({ teacher }: { teacher: Teacher }) {
  const telegramMessage = `Привіт! Хочу записатися до викладача ${teacher.name} на пробний урок. Підкажіть, будь ласка, найближчий вільний час.`;
  const teacherBookingLink = withTelegramText(MANAGER_TELEGRAM_URL, telegramMessage);

  return (
    <article className="flex h-[420px] min-w-0 flex-col rounded-[2rem] border border-border/60 bg-card/90 p-6 shadow-sm transition-all duration-150 sm:hover:border-primary/20 sm:hover:shadow-lg sm:hover:shadow-primary/[0.06]">
      ...
      <a
        ...
        className="mt-auto inline-flex w-full items-center justify-center gap-2 whitespace-nowrap rounded-xl bg-primary px-5 py-3 text-sm font-semibold text-primary-foreground transition-opacity duration-150 sm:hover:opacity-90"
      >
            ...
            {languageOrder.map((languageId) => {
              ...
              return (
                <button
                  ...
                  className={`rounded-xl px-4 py-2 text-sm font-semibold transition-all duration-100 ${
